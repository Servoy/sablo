...
/**
 * NOTE: this class can be used for keeping track of changes in any array-like structure, not just in the viewport of Servoy foundsets.
 *
 * This class receives a sequence of array (foundset viewport change) events (inserts/deletes/change by ref/updates) and processes them
 ...
 * the-fact) array (viewport) indexes.<br/><br/>
 *
 * That helps in that less changes might be sent to client/browser and that the client component, when
 * receiving them can rely on indexes from the already processed array (foundset viewport).<br/><br/>
 *
 * <b>For example:<br/>
 * <pre>
...
public class ArrayGranularChangeKeeper
{
	...
	private int numberOfUnchangedIndexes; // number of indexes who's data is not affected by any of the viewport operations; we monitor these because if there are too few of them it is more efficient to send the whole viewport then lots of changes

	public ArrayGranularChangeKeeper()
	{
	}
	...
	public void reset(int newViewportStart, int newViewportEnd)
	{
		...
		numberOfUnchangedIndexes = wholeUnchangedInterval.getUnchangedIndexesCount();
	...
	public boolean hasChanges()
	{
		...
		return initialViewportEnd != currentViewportEnd || (numberOfUnchangedIndexes != initialViewportEnd - viewportStart + 1);
	}

	public void processOperation(ArrayOperation operation)
	{
		if (operation.startIndex < viewportStart || operation.startIndex > currentViewportEnd + 1 ||
			(operation.endIndex > currentViewportEnd && operation.type != ArrayOperation.INSERT))
		...
		numberOfUnchangedIndexes = 0;
		while (listIterator.hasNext())
		{
			numberOfUnchangedIndexes += listIterator.next().applyOperation(operation, modifier);
		}

		// adjust size
		if (operation.type == ArrayOperation.DELETE) currentViewportEnd -= operation.endIndex - operation.startIndex + 1;
		else if (operation.type == ArrayOperation.INSERT) currentViewportEnd += operation.endIndex - operation.startIndex + 1;
	}

	public ArrayOperation[] getEquivalentSequenceOfOperations()
	{
		List<ArrayOperation> equivalentOps = new ArrayList<>(unchangedIntervals.size() * 2 + 4);
		...
		{
			appendEquivalentArrayOperations(previousUnchangedInterval, ui, equivalentOps);
			previousUnchangedInterval = ui;
		}

		// handle space after last unchanged interval
		appendEquivalentArrayOperations(previousUnchangedInterval,
			new UnchangedInterval(initialViewportEnd + 1, initialViewportEnd + 1, currentViewportEnd + 1, currentViewportEnd + 1), equivalentOps);

		return equivalentOps.toArray(new ArrayOperation[equivalentOps.size()]);
	}
	 ...
	 * previousUnchangedInterval and this interval (+ this interval in case of partially {@link UnchangedInterval}s; partially unchanged
	 * intervals could also get merged into previous equivalent partial op. if columnNames are the same and indexes are consecutive).
	 */
	protected void appendEquivalentArrayOperations(UnchangedInterval previousUnchangedInterval, UnchangedInterval currentUnchangedInterval,
		List<ArrayOperation> equivalentSequenceOfOperations)
	{
		// so changedCountInBetweenDelta =
		// number of original indexes that were between these 2 unchanged intervals
		//                              minus
		// number of final (after all changes were applied) indexes that are between these 2 unchanged intervals
		...
		{
			// new (unchanged) rows are less then previous rows => generate a delete OP
			equivalentSequenceOfOperations.add(new ArrayOperation(previousUnchangedInterval.getNewEnd() + 1,
				previousUnchangedInterval.getNewEnd() + changedCountInBetweenDelta, ArrayOperation.DELETE));
		...
		else if (changedCountInBetweenDelta < 0)
		{
			...
			equivalentSequenceOfOperations.add(new ArrayOperation(previousUnchangedInterval.getNewEnd() + 1,
				previousUnchangedInterval.getNewEnd() - changedCountInBetweenDelta, ArrayOperation.INSERT));
		...
		{
			equivalentSequenceOfOperations.add(new ArrayOperation(currentUnchangedInterval.getNewStart() - numberOfRowsToUpdate,
				currentUnchangedInterval.getNewStart() - 1, ArrayOperation.CHANGE));
		}

		// partially changed intervals might want to add some operations of their own
		currentUnchangedInterval.appendEquivalentArrayOperations(equivalentSequenceOfOperations);
	}

	/**
	 * This is useful to decide if it's worth sending the equivalent ArrayOperations or so much has changed that it's better to send the whole viewport...
	 * @return the number of unchanged rows in the viewport.
	 */
	public int getNumberOfUnchangedRows()
	{
		return numberOfUnchangedIndexes;
	}
